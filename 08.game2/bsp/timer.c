...
void Timer_config(void) {
	...
	#if 1
	//Timer1用作音频中断
	TIM_InitStructure.TIM_Value     = 0x10000 - 125;		//125us
...
u8 cur_task_id = 0;
/********************* Timer0中断函数************************/
void timer0_int(void) interrupt TIMER0_VECTOR INT_USING {
	ET0 = 0;
	led_scan();
...
/********************* Timer1中断函数************************/
void timer1_int(void) interrupt TIMER1_VECTOR INT_USING {
	ET1 = 0;
	CCAP0H = music_play();
	CCAP1H = CCAP0H;
	ET1 = 1;
...
/********************* Timer2中断函数************************/
void timer2_int(void) interrupt TIMER2_VECTOR INT_USING {
